...
export default async function ContractPage({
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="mb-4 text-3xl font-semibold">{contract.name}</h1>
      <p className="mb-6 text-gray-600">{contract.description}</p>

      <div className="mb-8">
        <div>Contract type: {contract.storage}</div>
        <div>
          Preview:{" "}
          <LinkButton href={`/c/contracts/${contract.id}`}>Go</LinkButton>
        </div>
        {contract.url && (
          <a
            href={contract.url}
