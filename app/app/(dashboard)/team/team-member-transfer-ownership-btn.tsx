...
export function TeamMemberTransferOwnershipBtn({ member, currentUserRole }: Props) {
  ...
  const canTransfer = currentUserRole === OrganizationRole.OWNER;

  // Can only transfer to non-pending, non-owner members and only if current user is owner
  if (isPendingInvite || isOwner || !canTransfer) {
    return null;
  }
