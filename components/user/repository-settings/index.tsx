...
export default function RepositorySettings({ repos: initialRepos }: { repos: Partial<Repo>[] }) {
  ...
  const handleAddAccount = useCallback(() => {
    const appInstallationWindow = window.open(
      `https://github.com/apps/${appName}/installations/select_target?state=${stateVariable}`,
      "_blank",
  ...
  useEffect(() => {
    ...
    filterTimeout = setTimeout(() => {
      ...
      setFilteredInstallationRepos(
        installationRepos.filter((repo) => repo.name.toLowerCase().includes(filter.toLowerCase()))
      );
    }, 500);
  }, [filter]);
    ...
    <div className="flex flex-col items-stretch gap-4">
      <div className="flex flex-row gap-4">
          ...
          <div className="flex flex-col items-start gap-4">
            <div className="w-full">
              ...
              <p className="text-sm text-stone-500">
                Your Github accounts and organizations in which you are a member.
              </p>
            ...
            <div className="relative w-full">
              ...
              {isPendingInstallationRepos ? <LoadingSpinner className="mx-auto" /> : null}
              {installations.find(({ id }) => `${id}` === currentInstallationId) &&
        ...
        <Card className="w-4/5 p-6 pt-5">
          ...
          <p className="text-sm text-stone-500">
            {" "}
            A Connected repository is a loose connection - this allows you to do research dependents
            your open source projects.
          </p>
          {repos.length === 0 && (
            <p className="text-sm text-stone-500">
              No connected repositories. Connect a Github account on the left and select
