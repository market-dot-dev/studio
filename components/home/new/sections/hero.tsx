...
export default function Hero() {
  return (
    <div className="mx-auto flex w-full max-w-[800px] flex-col items-center gap-6 xs:gap-8 px-6 tracking-tight  lg:max-w-[1300px] lg:px-12">
      <div className="flex h-full w-full max-w-[500px] flex-col items-center md:max-w-none">
        <GradientHeading className="xl:text-marketing-5xl text-[clamp(37px,9vw,72px)] md:text-marketing-4xl leading-[0.9] sm:leading-auto  mb-4 xs:mb-5 whitespace-nowrap text-center font-bold  tracking-[-0.035em] md:mb-6 md:tracking-[-0.045em] lg:mb-9">
        ...
        <p className="mb-6 max-w-[45ch] text-pretty text-center md:mb-8 md:text-[clamp(15px,12vw,24px)] md:leading-[clamp(16px,12vw,28px)]">
          Use Gitwallet to sell premium se<span className="tracking-normal">r</span>vices,
        ...
        <Link href={loginURL} className="md:mx-6 md:py-2">
          <Button>
            <Image
              ...
              height={24}
              width={24}
              className="h-[20px] xs:h-[22px] w-auto md:h-6"
            />
            Sign up with Github
          </Button>
        </Link>
      ...
      <div className="xs:-ml-1 xs:gap-4 -ml-2 flex w-fit items-center gap-3 sm:justify-center sm:gap-4 md:gap-6">
        <Image
          ...
          height={92}
          width={92}
          className="h-[clamp(16px,9.1vw,48px)] w-auto sm:h-12 md:h-14"
        />
        <Image
          src="/new-qs-logo.svg"
          alt="qs logo"
