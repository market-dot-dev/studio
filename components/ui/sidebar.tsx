...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "relative text-sidebar-accent hover:bg-sidebar-primary-foreground/10 hover:text-sidebar-primary-foreground focus:bg-sidebar-primary-foreground/10 focus:text-sidebar-primary-foreground",
...
const sidebarMenuButtonVariants = cva(
  "flex h-6 items-center gap-2 rounded px-1 text-sm font-medium transition-all duration-150 ease-in-out [&>svg]:size-[18px] [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: [
  ...
  (
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant }), "w-full", className)}
        {...props}
      >
        {children}
      </Comp>
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(
  ({ className, ...props }, ref) => (
    <ul
      ...
      className={cn("ml-6 space-y-1", "group-data-[collapsible=icon]:hidden", className)}
      {...props}
    />
  )
);
