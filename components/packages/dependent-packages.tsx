...
export default function DependentPackages({
  ...
  const getPackagesForRadarId = useCallback(
    (e: string) => {
      // clear the dropdowns
      setPackages([]);
  ...
  const getDependentsForPackageId = useCallback(
    (e: string) => {
  ...
  const getVersionDependents = useCallback(
    (e: any) => {
      ...
      startDependentsTransition(() => {
        getDependentPackages(parseInt(selectedPackage), {
          [key]: value.replace("v", ""),
        }).then((res) => {
  ...
  const chartSection = (
    <>
      {isPendingPackages ? (
        ...
        <BarChart
          className="mx-auto mt-4 h-72 w-full pt-0"
    ...
    <div className="mt-6 flex flex-col items-stretch gap-6">
      {!repos.length ? (
        <Text>
          ...
          to see real usage data for your repos & packages.
        </Text>
      ) : null}
      <div className="relative z-20 grid grid-cols-3 gap-6">
          ...
          >
            {repos.length ? (
              repos.map((repo: RepoItem) => (
                <SelectItem key={repo.radarId} value={`${repo.radarId}`}>
                  {repo.name}
                </SelectItem>
              ))
            ) : (
        ...
        <div className="grow">
          ...
          >
            <TabList variant="solid">
              {Object.keys(allData ?? dummyFacetsData).map(
                (key: string, index: number) => (
                  <Tab
                    value={key}
                    key={index}
                    className={tabIndex === index ? "bg-white" : ""}
                  >
                    {versionLabels[key]}
                  </Tab>
                ),
      ...
      {compact ? (
        ...
        <Card className="z-10 min-h-12 w-full bg-white">{chartSection}</Card>
      )}
      {chartData.length ? (
              ...
              <Table className="w-full">
                <TableHead>
                  <TableRow>
                    ...
                    <TableHeaderCell>Latest Release</TableHeaderCell>
                    <TableHeaderCell>Downloads</TableHeaderCell>
                    <TableHeaderCell>Resolved Versions</TableHeaderCell>
                  </TableRow>
                </TableHead>
                <TableBody>
...
function PackageRow({ data }: { data: any }) {
  ...
  return (
    <TableRow key={data.id} className="text-gray-700">
      ...
      <TableCell>
        <p>
          {data.package_fields.latest_release_number ?? ""} (
          {new Date(
            data.package_fields.latest_release_published_at,
          ).toLocaleDateString()}
          )
        </p>
      </TableCell>
      ...
      {payload.map((category: any, idx: number) => (
        <div key={idx} className="flex flex-1 space-x-2.5">
          ...
          <div className="space-y-1">
            ...
            <p className="font-small">Click for more details</p>
          </div>
        </div>
      ))}
    </div>
