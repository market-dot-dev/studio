...
function TodoItem({ title, children, step, currentStep, pathName, completedSteps, setCompletedSteps} : any) : JSX.Element {
    ...
    useEffect(() => {
        setOpen(step === currentStep && !completedSteps[step]);
        ...
        <div className={ cardClasses }>
            <div className="p-2 cursor-pointer" onClick={togglePanel}>
                <Flex alignItems="center" justifyContent="start" className="gap-3">
                    ...
                    <Bold>{title}</Bold>
                    { open ? <FaChevronUp className="ml-auto text-gray-400 scale-90" /> : <FaChevronDown className="ml-auto text-gray-400 scale-90" /> }
                ...
                <Accordion defaultOpen={true}>
                    ...
                    <AccordionBody className="p-2">
                        <Flex flexDirection="col" alignItems="start" className="gap-1">
                            ...
                            <Flex justifyContent="end" alignItems="center" className="gap-4 mt-2 mr-2">
                                <Button icon={IoIosClose} loading={isDismissing} disabled={isDismissing} variant="light" iconPosition="right" onClick={dismissGuide}>Dismiss</Button>
                            </Flex>
                        </Flex>
                    </AccordionBody>
                </Accordion>
