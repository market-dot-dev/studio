...
import {
  createStripePriceById,
  destroyStripePriceById,
} from "@/app/services/TierService";
import { useState } from "react";

const TierPriceWidget = ({
  tierId,
  price: iPrice,
  stripePriceId: iStripePriceId,
}: {
  tierId: string;
  price: number;
  ...
  return (
    <>
      ...
      <NumberInput name="price" placeholder={"price"} value={price || 0} />

      {stripePriceId ? (
        <Button
          onClick={async () => {
            destroyStripePriceById(tierId);
          }}
        >
          Destroy
        </Button>
