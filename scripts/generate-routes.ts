...
async function scanRoutes(dir: string, baseRoute: string = ""): Promise<RouteInfo[]> {
  ...
  for (const file of items) {
    ...
    if (stat.isFile() && ROUTE_FILES.includes(file)) {
      ...
      const routeSegment = isDynamicSegment ? `:${paramName}` : file;
...
async function generateRoutes() {
  ...
  const routesArray = Object.entries(uniqueRoutes).map(([path, info]) => ({
    path,
  ...
  for (const route of routesArray) {
    ...
    const existingRouteIndex = routesByNamespace[namespace].findIndex(
      (r) =>
        r.path === route.path &&
        JSON.stringify(r.methods.sort()) === JSON.stringify(route.methods.sort())
    );
